...
#     word2 = pair_dict[i][1]
#     if (word1 in vec_dict and word2 in vec_dict):
#         cs = cosine_similarity(vec_dict[word1],vec_dict[word2])
#         similar_dict[word1 , word2] = cs
#
...
if word  in vec_dict:
    for vec in vec_dict:
        similar_dict[vec] = cosine_similarity(vec_dict[word], vec_dict[vec])

    similar_dict = sorted(similar_dict.items(), key=lambda x:x[1], reverse=True)
print(similar_dict[1:k+1])
